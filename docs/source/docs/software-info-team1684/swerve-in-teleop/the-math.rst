...
we define three command inputs - Forward, Strafe, and Rotation, or :math:`FWD`, :math:`STR`, and :math:`ROT`. Let's
...
one stick controls translation rate ( :math:`FWD` and :math:`STR`) and the other's left-right axis controls rotation
