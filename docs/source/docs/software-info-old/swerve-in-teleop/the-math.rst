...
We define FWD and STR as the desired forward/back and strafe commands from the joystick,
relative to the field. These can both range from -1 to 1 from the joystick, assuming things are filtered
correctly, and the magnitude of the input vector (FWD, STR) shouldn’t ever be greater than 1. We’ll
...
transform our FWD and STR commands as follows:

FWDnew = FWD * cos(θ) + STR * sin(θ)
STRnew = STR * cos(θ) − FWD * sin(θ)

First, we calculate our new FWD, which can be thought of as simply figuring out how much of each
original command (FWD and STR) are in the actual direction of the robot’s “FWD”. Likewise, we can
do the same for our STR command. This effectively transforms our commands, considered relative
...
this as the result of a rotation matrix , applied to the vector (FWD, STR).
...
changes as a result. Here, since we’re aligned with the field, θ=0°. Let’s check our sine and cosine
...
cos(0°) is 1, and sin(0°) is 0. If we think about a right triangle, with one of its angles being 0° and a
...
FWDnew = FWD * cos(90°) + STR * sin(90°) = FWD * 0 + STR * 1 = STR
STRnew = STR * cos(90°) − FWD * sin(90°) = STR * 0 − FWD * 1 = − FWD
...
F W D F W D os(30°) F new = * c + STR * sin(30°) = FWD * 0.866 + STR * 0.5
STRnew = STR * cos(30°) − FWD * sin(30°) = STR * 0.866 − FWD * 0.5

Both FWD and STR contribute to each of the transformed values. This pattern will carry through for
...
Now that we have our FWD and STR commands transformed, we need to figure out what each wheel
...
as L and W, and let’s define an additional value R, as
R = √L2 + W2 , or the diameter of the circle that contacts all
